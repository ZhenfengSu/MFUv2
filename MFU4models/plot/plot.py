    ...
    with open(output_path, 'r') as f:  
        ...
        for line in all_lines:  
            if 'batch_size' in line:  
                ...
                MFU_end_index = line.index(' THROUGHPUT')  
                MFU = float(line[MFU_begin_index + 5:MFU_end_index])  
                THROUGHPUT_begin_index = line.index('THROUGHPUT: ')  
                THROUGHPUT_end_index = line.index(' FLOPS')  
                THROUGHPUT = float(line[THROUGHPUT_begin_index + 11:THROUGHPUT_end_index])  
            ...
            if 'model_type' in line:  
                model_type_index = line.index('model_type: ')  
                model_type = line[model_type_index + 12:-1]  
    return BATCH_SIZE_list, MFU_list, THROUGHPUT_list, FLOPS_model_list, model_type  

def plot_MFU_THROUGHPUT_FLOPS(output_path, save_path):  
    batch_size_list, MFU_list, THROUGHPUT_list, FLOPS_model_list, model_type = get_output_info(output_path)  
    ...
    # Plot Throughput  
    plt.subplot(3, 1, 2)  
    plt.plot(batch_size_list, THROUGHPUT_list, 'go-', label='Throughput')  
    plt.xlabel('Batch Size', fontsize=12)  
    plt.ylabel('Throughput (images/s)', fontsize=12)  # Add images/s unit  
    ...
    # Plot FLOPS Model  
    plt.subplot(3, 1, 3)  
    plt.plot(batch_size_list, [f / 1e12 for f in FLOPS_model_list], 'bo-', label='FLOPS Model')  # Convert to TFLOPS  
    plt.xlabel('Batch Size', fontsize=12)  
...
def get_args_parser():  
    ...
    parser.add_argument("--output-path", type=str, default='output_resnet.txt', help='output_path')  
    parser.add_argument("--save-path", type=str, default='output_resnet.png', help='save_path')  
