...
'''
@register_model
def vanillaformer_large_patch16(pretrained=False, **kwargs):
    model = VisionLLaMA(patch_size=16,embed_dim=4096,depth=1,num_heads=32,mlp_ratio=4.,qkv_bias=True,
                        norm_layer=partial(nn.LayerNorm, eps=1e-6), blk_cls=LAMABlock, share_rpe=True,
                        use_cls_token=True, use_abs=False, auto_scale=True,  **kwargs)
    model.default_cfg = _cfg()
    return model
...
if __name__ == '__main__':
    ...
    # model = pllama_wols_large_patch16()
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            params += param.numel()
    print(params)
    print("the model has {} M params".format(params / 1e6))
